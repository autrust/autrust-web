...
function getPhotoForVehicle(title, make, model, category) {
  ...
  if (titleLower.includes("série 3") && titleLower.includes("touring") || titleLower.includes("serie 3") && titleLower.includes("touring") || (titleLower.includes("bmw") && (titleLower.includes("série 3") || titleLower.includes("serie 3")) && titleLower.includes("touring"))) {
    return "/uploads/bmw-serie-3-touring.png";
  }
  
  // BMW Série 3 (berline)
  if (titleLower.includes("série 3") || titleLower.includes("serie 3") || (titleLower.includes("bmw") && (titleLower.includes("série 3") || titleLower.includes("serie 3")))) {
...
async function main() {
  ...
  console.log("🔄 Mise à jour FORCÉE de toutes les photos pour correspondre aux véhicules...");

  // Récupérer toutes les annonces actives avec leurs photos
  ...
  for (const listing of allListings) {
    const correctPhotoUrl = getPhotoForVehicle(listing.title, listing.make, listing.model, listing.category);
    ...
    console.log(`✅ Photo mise à jour pour: ${listing.title} (${listing.make || 'N/A'} ${listing.model || ''})`);
    updatedCount++;
  }

  console.log(`\n🎉 ${updatedCount} annonce(s) mise(s) à jour avec des photos correspondantes !`);
