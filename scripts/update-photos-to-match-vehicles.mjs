...
async function main() {
  ...
  console.log("🔄 Mise à jour des photos pour correspondre aux véhicules...");
  ...
  for (const listing of allListings) {
    ...
    // Si l'annonce n'a pas de photos, en ajouter une
    if (!listing.photos || listing.photos.length === 0) {
      ...
      // Vérifier si la première photo correspond au véhicule
      const firstPhoto = listing.photos.sort((a, b) => a.order - b.order)[0];
      if (firstPhoto.url !== correctPhotoUrl) {
        // Mettre à jour la première photo
        await prisma.listingPhoto.update({
          where: { id: firstPhoto.id },
          data: { url: correctPhotoUrl },
        });
